...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "-------------------------\n");
	bt_device_table_t device_table = hci_scan_devices(&hci_socket, 5, 10, IREQ_CACHE_FLUSH);
	bt_device_table_display(device_table);
	free(device_table.device);

	fprintf(stderr, "\n-----------------------------\n");
	fprintf(stderr, "- GETING RSSI MEASURES TEST -\n");
	fprintf(stderr, "-----------------------------\n");
	hci_get_RSSI(NULL, NULL, NULL, 5, 20);*/
	...
	hci_LE_get_RSSI(&hci_socket1, NULL, NULL, num_points, 0x00, scan_interval, scan_window, 0x00, 0x00);	
